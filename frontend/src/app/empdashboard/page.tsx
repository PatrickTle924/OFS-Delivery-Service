...
export default function DashboardPage() {
  ...
  const fetchInventoryData = async () => {
    try {
      ...
      const lowStock = data.filter((item: any) => item.quantity <= item.reorderLevel);
  ...
  return (
    <div className="flex min-h-screen bg-cream font-dm">
      ...
      <div className="flex-1 p-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="font-playfair text-4xl text-forest mb-2">Welcome, {userName}!</h1>
        ...
        <div className="grid grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-xl p-6 shadow-md border-l-4 border-sage">
            <p className="text-[#666] text-sm font-medium mb-2">Pending Orders</p>
            <p className="font-playfair text-3xl text-forest">{pendingOrders}</p>
          </div>
          <div className="bg-white rounded-xl p-6 shadow-md border-l-4 border-mint">
            <p className="text-[#666] text-sm font-medium mb-2">Completed Today</p>
            <p className="font-playfair text-3xl text-forest">{completedOrders}</p>
          </div>
          <div className="bg-white rounded-xl p-6 shadow-md border-l-4 border-warm">
            <p className="text-[#666] text-sm font-medium mb-2">Low Stock Items</p>
            <p className="font-playfair text-3xl text-forest">{lowStockItems.length}</p>
          </div>
          <div className="bg-white rounded-xl p-6 shadow-md border-l-4 border-blue-400">
            <p className="text-[#666] text-sm font-medium mb-2">Total Revenue</p>
        ...
        <div className="grid grid-cols-3 gap-8">
          ...
          <div className="col-span-2">
            <div className="bg-white rounded-xl shadow-md p-6 mb-8">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-playfair text-xl text-forest">Low Stock Items</h2>
                <Link
                ...
                <div className="space-y-3">
                  {lowStockItems.map((item) => (
                    <div key={item.id} className="flex items-center justify-between p-3 bg-red-50 rounded-lg border border-red-200">
                      <div>
                        <p className="font-medium text-[#1a1a14]">{item.name}</p>
                        <p className="text-xs text-[#666]">Reorder Level: {item.reorderLevel}</p>
                      </div>
                      <div className="text-right">
                        <p className="font-bold text-red-700 text-lg">{item.quantity}</p>
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-playfair text-xl text-forest">Today's Orders</h2>
                <Link
                  href="/orders"
                  className="text-sage font-medium text-sm hover:text-forest transition-colors"
                >
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[#ddd]">
                      <th className="text-left py-3 px-4 font-medium text-forest">Order ID</th>
                      <th className="text-left py-3 px-4 font-medium text-forest">Customer</th>
                      <th className="text-left py-3 px-4 font-medium text-forest">Status</th>
                      <th className="text-right py-3 px-4 font-medium text-forest">Total</th>
                    </tr>
                  </thead>
                  <tbody>
                    {orders.slice(0, 5).map((order) => (
                      <tr key={order.id} className="border-b border-[#ddd] hover:bg-cream/50">
                        <td className="py-3 px-4 font-medium text-[#1a1a14]">{order.id}</td>
                        <td className="py-3 px-4 text-[#666]">{order.customerName}</td>
                        <td className="py-3 px-4">
                          <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium capitalize ${getStatusBadgeColor(order.status)}`}>
                            {order.status}
                          </span>
                        </td>
                        <td className="py-3 px-4 text-right font-medium text-[#1a1a14]">${order.total.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div>
            <div className="bg-white rounded-xl shadow-md p-6">
              <h2 className="font-playfair text-xl text-forest mb-4">Quick Actions</h2>
              ...
              <div className="space-y-3">
                ...
                >
                  <p className="font-medium text-forest text-sm font-bold">Manage Inventory</p>
                </Link>
                ...
                >
                  <p className="font-medium text-forest text-sm font-bold">Create Order</p>
                </Link>
                ...
                >
                  <p className="font-medium text-forest text-sm font-bold">View Reports</p>
                </Link>
                ...
                >
                  <p className="font-medium text-forest text-sm font-bold">Schedule Delivery</p>
                </Link>
              </div>
            </div>
            ...
            <div className="bg-sage/10 rounded-xl p-6 mt-6 border border-sage/20">
              <p className="text-sm text-forest">
                <strong>Need help?</strong> Check our documentation or contact support.
              </p>
            </div>
          </div>
        </div>
