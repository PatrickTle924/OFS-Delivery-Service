...
export default function OrderCard({ order }: { order: OrderData }) {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem("ofsUser");
      if (stored) {
        const user = JSON.parse(stored);
        if (user?.customerId) setCustomerId(user.customerId);
      }
    } catch {
      // no-op
    }
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-white/90 p-6 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="text-[#b94040] text-sm font-medium px-5 py-2.5 rounded-xl border border-clay/20 hover:bg-clay/10 transition-colors"
          >
            Report Issue
          </button>
